# this code is follows the idea of https://github.com/fuzhenxin/text_style_transfer to compute CPP
# author somaye Jan 2020
# !//env python2
# encoding=utf-8
import sys
import numpy as np
...
def load_arguments():
    ...
    argparser.add_argument('--file_2',
            type=str,
            default='')
    argparser.add_argument('--file_gen_mode',
            type=str,
            default='') # 'rec' or 'tsf', if '', the file is a gold file
...
def gen_score(gen_dir_name, src_dir_name,  file_dir_write, file_gen_mode=''):
    emb = Embedding(100)
...
def gen_score_random(file_name_1, file_name_2, file_dir_write, file_gen_mode=''):
    emb = Embedding(100)
    word_dict = emb.get_all_emb()
    res = []
    styles = ['.neg', '.pos']
...
if __name__=="__main__":
    ...
    file_gen_mode = args.file_gen_mode 
    print('Content Preservation Power: CPP')
    if args.random_files:
        print("Comparing random files ...")
        res, avg_res = gen_score_random(file_1_dir_name, file_2_dir_name, output_dir_name) 
        print(F"CPP of the random files {file_1_dir_name} and {file_2_dir_name} with style 1: {res[0]} and with style 2: {res[1]}")
        print(F"Average of CPP: {avg_res}")
    else:
        print("Comparing source and generated files ...")
        res, avg_res = gen_score(file_1_dir_name, file_2_dir_name, output_dir_name, file_gen_mode) 
        print(F"CPP of the source file {file_2_dir_name} and generated file {file_2_dir_name} with style 1: {res[0]} and with style 2: {res[1]}")
        print(F"Average of CPP: {avg_res}")
